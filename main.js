...
Invoice.prototype = {
    ...
    calcTotal: function () {
         ...
         jQuery($.opt.parentClass).each(function (i) {
             var row = jQuery(this);
             var priceval = $("input[name='soleprice']").val();
             var total = priceval * row.find($.opt.qty).val();
    ...
    calcGrandTotal: function () {
        ...
        var discount = Number(jQuery($.opt.discount).val());
        var totaldiscount = (discount*subtotal)/100;
    ...
    newRow: function () {
        ...
        var base = $("input[name='sole']:checked").val();
        var above = $("input[name='strip']:checked").val();
        var prosize = $("input[name='size']:checked").val();
        var price = $("input[name='soleprice']").val();
        console.log(price);

        jQuery(".item-row:last").after('<tr class="item-row"><td class="item-name" style="color: black;">' + base + ' SOLE ' + above + ' STRIP ' + prosize + '<div class="delete-btn"><a class=' + $.opt.delete.substring(1) + ' href="javascript:;" style="color:red;font-weight:bold;" title="Remove row">X</a></div></td><td><p class="form-control price" placeholder="Price" type="text"> ' + price + ' </p></td><td><input class="form-control qty" placeholder="Quantity" type="text"></td><td><span class="total">0.00</span></td></tr>');
